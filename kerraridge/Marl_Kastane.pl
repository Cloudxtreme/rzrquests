...
sub EVENT_ITEM {
  $faction = $client->GetCharacterFactionLevel(342);
  if (($faction >= 43) && plugin::check_handin(\%itemcount, 14376 => 1)) { # Note to Marl
  ...
  if (($faction >= 51) && plugin::check_handin(\%itemcount, 14381 => 1)) { # Blood of Kyrenna
    ...
    quest::faction(342, 7);
    quest::summonitem(14377); # Dark Shroud
  }
  if (($faction >= 58) && plugin::check_handin(\%itemcount, 14379 => 1)) { # Glohnor wrappings
    ...
    quest::faction(342, 7);
    quest::summonitem(14370); # Will of Innoruuk
  }
